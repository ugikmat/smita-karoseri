...
@section('content_header')
    <h1>List Invoice Pengembalian SP</h1>
@stop
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">Input Pengembalian SP</h4>
      </div>
...
<div class="modal fade" id="deleteModal">
  <div class="modal-dialog">
    <div class="modal-content">
      <form id="deleteForm" action="/pengembalian_sp/delete" method="POST">
        @csrf @method('put')
        <input type="hidden" name="id" id="id_pengembalian" value="">
        <!-- Modal Header -->
        <div class="modal-header">
...
<script>
    $(function () {
      @if(Session::has('sp-list-sales'))
        ...
        @endif
        @if(Session::has('lokasi_pengembalian'))
          $('#lokasi').val("{{session('lokasi_pengembalian')}}").change();
        @endif
        ...
        var t = $('#list-invoice-table').DataTable({
            ...
            ajax: `/smita/pengembalian_sp/list/${$tgl_awal}/${$tgl_akhir}/${$lokasi}/${$sales}`,
            // "columnDefs": [ {
            // "searchable": false,
            // "orderable": false,
            // "targets": 0
        ...
        $sales = $('#sales').val();
          ...
          t.ajax.url(`/smita/pengembalian_sp/list/${$tgl_awal}/${$tgl_akhir}/${$lokasi}/${$sales}`).load();
          console.log('Loaded');
        });
        ...
        $('#verificationModal').on('show.bs.modal', function (event) {
          var button = $(event.relatedTarget) // Button that triggered the modal
          var id = button.data('id'); // Extract info from data-* attributes
          $('#verificationForm').attr('action',`/smita/pengembalian_sp/verify/${id}`);
        });
    });
</script>
@stop
