...
<div class="container-fluid form-inline">
  <form class="invoice-sp repeater" action="{{ url('/') }}/persediaan/sp/mutasi-sp-semua-cvs/show" method="post">
  @csrf
  <div class="row">
...
<script>
    $(function () {
        ...
        var t = $('#mutasi-sp-semua-cvs-table').DataTable({
            ...
            ajax: `{{ url('/') }}/stok-sp/all/data/${$tgl_awal}/${$tgl_akhir}`,
            columns: [
              // {data: 'indeks'},
              {data: 'nama_produk'},
              @isset($saless)
        ...
        $tgl_awal = $('#tgl_awal').val();
          $sales = $('#sales').val();
          t.ajax.url(`{{ url('/') }}/stok-sp/all/data/${$tgl_awal}/${$tgl_akhir}`).load();
        });
    });
</script>
@stop
