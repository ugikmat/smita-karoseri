...
<br>
<form action="{{ url('/') }}/pembelian/dompul/list/store" method="post" class="repeater">
  @csrf
    ...
    $(function () {
        ...
        @if($pembelianDompul->status_pembayaran==0)
          var t = $('#list-edit-invoice-table').DataTable({
            ...
            processing: true,
            searching:  false,
            ajax: `{{ url('/') }}/pembelian_dompul/detail/${id_pembelian}`,
            columns: [
              {data: 'produk'},
              {data: 'tipe_harga'},
              {data: 'harga_satuan'},
      ...
      //ajax call
        ...
        .done(function(response){
          if(response.success)
          {
            ...
            t.ajax.url(`{{ url('/') }}/pembelian_dompul/detail/${id_pembelian}`).load();
...
<script>
  $('#editModal').on('show.bs.modal', function (event) {
    ...
    console.log(produk);
    $('#link').val(`{{ url('/') }}/pembelian/dompul/list/update/${id}`);
  })
</script>
@stop
