...
@section('content')
<form class="invoice-dompul repeater" action="{{ url('/') }}/pembelian/dompul/verify" method="post">
...
for (let index = 0; index < {{$dompuls->count()}}; index++) {
  $(`#tipe${index+1}`).on('change',function (event) {
    //ajax call
    $.post('{{ url('/') }}/dompul/get_harga/'+$(this).val()+'/'+$(`#nama${index+1}`).val(), function(response){
    if(response.success)
    {
      totalHarga-=parseFloat(harga[index]);
