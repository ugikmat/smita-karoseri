              ...
              <div class="x_content">
                ...
                <form id="editForm" method="POST" data-parsley-validate class="form-horizontal form-label-left" action="">
                  ...
                  <div class="form-group row">
                    ...
                    @else
                      <input class="datepicker col-md-7 col-xs-12" id="tgl" data-date-format="dd-mm-yyyy" value="{{Carbon\Carbon::now('Asia/Jakarta')->format('d-m-Y')}}">
                    @endif
                    </div>
                  </div>
  ...
  $.ajaxSetup({
    ...
    $(function () {
        ...
        var t = $('#lp-dompul-table').DataTable({
            ...
            stateSave: true,
            scrollX: true,
            ajax: `{{ url('/') }}/laporan-penjualan/dompul-cvs/${$tgl}/${$sales}`,
            // columnDefs: [
            //     {
            //         targets:0,
        ...
        $.post(`{{ url('/') }}/get_laporan_dompul_cvs/${$tgl}/${$sales}`, function(response){
            if(response.success)
            {
              console.log('Success..');
        ...
        $('#save').on('click',function(event) {
          $tgl = $('#tgl').val();
          $sales = $('#sales').val();
          t.ajax.url(`{{ url('/') }}/laporan-penjualan/dompul-cvs/${$tgl}/${$sales}`).load();
          $.post(`{{ url('/') }}/get_laporan_dompul_cvs/${$tgl}/${$sales}`, function(response){
            if(response.success)
            {
              console.log('Success..');
